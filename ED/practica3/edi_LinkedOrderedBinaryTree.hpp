/**
@file edi_LinkedOrderedBinaryTree.hpp
@brief Fichero con arbol binario ordenado linkado
@author Ruben
*/
...
public:
	...
	* @arg[in] x is the value to be searched.
	*/
	virtual T buscaPosicion(const T& x)
	{
		assert(checkInvariants()&& ++_invariantCount);
		T elem;
		bool found=false;
...
//     }
	...
	*/
	...
	bool imprimeEnOrden(const LinkedOrderedBinaryTree<T> * t){
		
		bool retorno=true;

		if(not t->isEmpty()){
			if(t->hasLeftChild()){
				std::auto_ptr<const LinkedOrderedBinaryTree<T> > ptr(t->leftSubTree());
				retorno=imprimeEnOrden(ptr.get());
			}
			if(retorno){
				printf("%d ) %d\n", (t->item()).getPosicion(), (t->item()).getDni());
			}
			if (retorno && t->hasRightChild())
			{
				std::auto_ptr<const LinkedOrderedBinaryTree<T> > ptr(t->rightSubTree());
				retorno=imprimeEnOrden(ptr.get());
			}
	...
	bool guardaFichero(const LinkedOrderedBinaryTree<T> * t, const char * fichero){
		bool retorno=true;

		ofstream f;
		f.open(fichero,std::ofstream::app);
		if(f.is_open())
			f<<t;
  		f.close();
	...
	bool cargaFichero(LinkedOrderedBinaryTree<T>* t, const char * fichero){
		ifstream f;
	...
	friend std::ofstream & operator<<(std::ofstream& out, const LinkedOrderedBinaryTree<T>* t){
		if(!t->isEmpty()){
			out<<"(";
			out<<t->item();
			out<<":";
			...
			out<<")";
  			...
  			out<<"El arbol esta vacio"<<endl;
  		}
  		return out;
	}

	friend std::ifstream & operator>> (std::ifstream& in, LinkedOrderedBinaryTree<T>* t){
		string value;
		T c;
		char aux;
		...
		if(aux=='('){
			getline(in, value, ',');
			c.setDni(atoi(value.c_str()));
			getline(in, value, ':');
			c.setPosicion(atoi(value.c_str()));
			t->insert(c);
			in>>t;
			in>>t;
			in>>aux;	
...
protected:  
	...
	*/  
	...
	/**
	@class CheckOrdering
	@brief This class is used as example of Functor to check the invariant (inorderBinaryTreeProcessor is ordered).
