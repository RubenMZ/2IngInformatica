	...
	class Persona
	{
		...
		public:
			...
			*/			
			...
			/** 
				@fn     friend std::ofstream & operator<<( std::ofstream &stream, const edi::Persona &p);
				@brief  Escribe los datos de una persona en un fichero de texto
				@note   Sobrecarga del operador de salida "<<"
				@param	stream Fichero de salida
				@param  p Persona, cuyos datos se escriben en el fichero
				@return Fichero en el que se han escrito los datos
			*/

			friend std::ofstream & operator<<( std::ofstream &stream, const edi::Persona &p);

			/** 
				@fn     friend std::ifstream & operator>>( std::ifstream &stream, edi::Persona &p);
				@brief  Lee los datos de una persona desde un fichero de texto
				@note   Sobrecarga del operador de salida ">>"
				@param	stream Fichero de entrada
				@param  p Persona, cuyos datos se leen desde el fichero
				@return Fichero del que se leen los datos
			*/


			friend std::ifstream & operator>>( std::ifstream &stream, edi::Persona &p);
