	...
	public:
			...
			*/
			void imprimirAscendente(){
				...
				for(int i=0;(i<numeroAlumnos_); i++){
					...
					cout<<"Alumno: "<<(alumno.apellido())<<", "<<(alumno.nombre())<<" con DNI: "<<alumno.dni()<<endl;
				}
			}
			/**
			...
			*/
			void listarEntreDosAscendente( Persona p1,  Persona p2){
				...
				if(p1==p2){
					cout<<"Alumno: "<<(p1.apellido())<<", "<<(p1.nombre())<<" con DNI: "<<p1.dni()<<endl;
				}else{
				for(int i=0;(i<numeroAlumnos_); i++){
					aux=alumnos_.item(i);
					if(aux>=p1 && aux<=p2){
						cout<<"Alumno: "<<(aux.apellido())<<", "<<(aux.nombre())<<" con DNI: "<<aux.dni()<<endl;
					}
				}
				}
			}
