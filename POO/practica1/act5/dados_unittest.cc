...
TEST(Dados, NumLanzamientos){
	...
	EXPECT_EQ(4, d.getLanzamientos2());
...
TEST(Dados, Media){
	...
	EXPECT_LT(d.getMedia1(), 1);

	d.setDado1(3);
	d.setDado1(2);

	EXPECT_EQ(2.5, d.getMedia1());
...
TEST(Dados, Ultimos){
	...
	for(int i=0; i<100; i++){
		...
		EXPECT_EQ(vector1[d.getLanzamientos1()%5], d.getDado1());
