...
TEST(Ruleta, Constructor) {
  ...
  EXPECT_EQ("codigo1", r.getCrupier().getCodigoCrupier());
...
TEST(Ruleta, getCrupier) {
  ...
  EXPECT_EQ("codigo1", d.getCodigoCrupier());
}
...
TEST(Ruleta, addJugador) {
  ...
  EXPECT_EQ("jugador1", r.getJugadores().begin()->getCodigoJugador());
  EXPECT_EQ("55XX", (--r.getJugadores().end())->getDNI());
  EXPECT_EQ("jugador2", (--r.getJugadores().end())->getCodigoJugador());
}
...
TEST(Ruleta, deleteJugador) {
  ...
  EXPECT_EQ("jugador2", r.getJugadores().begin()->getCodigoJugador());
  EXPECT_EQ(1, r.getJugadores().size());
  j1.setDNI("88XX");
...
TEST(Ruleta, escribeLeeJugadores) {
  ...
  EXPECT_EQ("jugador1", r2.getJugadores().begin()->getCodigoJugador());
  EXPECT_EQ("55XX", (--r2.getJugadores().end())->getDNI());
  EXPECT_EQ("jugador2", (--r2.getJugadores().end())->getCodigoJugador());
}
